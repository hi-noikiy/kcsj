...
namespace HTProject.Pages.RG_QYZiZhi
{
	...
	public partial class RG_QYZiZhi_List : Epoint.Frame.Bizlogic.BaseContentPage_UsingMaster 
	{
		...
		protected void Page_Load(object sender, System.EventArgs e)
		{
			if (!Page.IsPostBack )
			{
                ...
                {
                    string url = "window.location.href='RG_QYZiZhi_List.aspx";
		...
		protected void btnOK_Click(object sender, System.EventArgs e)
		{
            ...
            //{
            //    //重新加载
            //    string url = "window.location.href='RG_QYZiZhi_Edit.aspx";
            //    if (!String.IsNullOrEmpty(Request["DWGuid"]))
            //    {
        ...
        private void FirstGrid()
        {
            ...
            if (DWName.Text.Trim() != "")
            {
                oListPage.OtherCondition += " AND EXISTS(SELECT 1 FROM RG_OUInfo OU WHERE OU.ROWGUID=RG_QYZiZhi.DWGUID AND EnterpriseName LIKE '%" + Epoint.MisBizLogic2.DB.SQL_Encode(DWName.Text) + "%') ";
            }
            if (ZZLB.Text.Trim() != "")
            {
                oListPage.OtherCondition += "  AND ZiZhiText LIKE '%" + Epoint.MisBizLogic2.DB.SQL_Encode(ZZLB.Text) + "%' ";
            }
		...
		protected void btnDel_Click(object sender, System.EventArgs e)
		{
            ...
            for (int i = 0; i < Datagrid1.Items.Count; i++)
            {
                ...
                if (chk.Checked)
                {
                    //如果是编辑中或退回的，可直接删除，如果是已经通过的，只改变状态
                    ...
                    if (oRow["Status"].ToString() == "90")//审核通过
                    {
