...
namespace HTProject.Pages.RG_QYZiZhi
{
    ...
    public partial class RG_QYZiZhiAD_List : Epoint.Frame.Bizlogic.BaseContentPage_UsingMaster
    {
        ...
        private void RefreshGrid()
        {
            ...
            if (DWName.Text.Trim() != "")
            {
                oListPage.OtherCondition += " AND EXISTS(SELECT 1 FROM RG_OUInfo OU WHERE OU.ROWGUID=RG_QiYeZiZhi.DWGUID AND EnterpriseName LIKE '%" + Epoint.MisBizLogic2.DB.SQL_Encode(DWName.Text) + "%') ";
            }
            if (ZZLB.Text.Trim() != "")
            {
