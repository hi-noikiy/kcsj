<%@ Page Language="C#" MasterPageFile="~/WebMaster/ContentPageNoTop.master" AutoEventWireup="true"
    Inherits="EpointRegisterUser.Pages.SelDWForPro" Title="单位选择列表" CodeBehind="SelDWForPro.aspx.cs" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <script>
        function SelDW(DWGuid, name) {
            if (window.confirm("确定选择?")) {
                document.getElementById("<%=hiDWGuid.ClientID %>").value = DWGuid;
                document.getElementById("<%=hiDWName.ClientID %>").value = name;
                document.all("<%=btnYes.ClientID %>").click();
            }
        }
        //-->
    </script>
                            ...
                            <table width="100%">
                                <tr>
                                    ...
                                    <td class="TableSpecial" width="35%" style="height: 30px;" align="left">
                                        ...
                                        <input type="hidden" runat="server" id="hiDWGuid" />
                                        <input type="hidden" runat="server" id="hiDWName" />
                                        <asp:Button CausesValidation="false" ID="btnYes" runat="server" onclick="btnYes_Click" style="display:none" />
                                    </td>
                                </tr>
                            </table>
                        </td>
